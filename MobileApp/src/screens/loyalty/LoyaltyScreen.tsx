...
export default function LoyaltyScreen() {
  ...
  const { data: transactionsData, isLoading: transactionsLoading, isFetching, error: transactionsError } = useQuery({
    queryKey: ['loyaltyTransactions', page],
    queryFn: () => loyaltyApi.getTransactions({ page, limit: 20 }),
    retry: 2,
  });
  ...
  const onRefresh = async () => {
    setRefreshing(true);
    try {
      await queryClient.invalidateQueries({ queryKey: ['loyaltyAccount'] });
      await queryClient.invalidateQueries({ queryKey: ['loyaltyTransactions'] });
      setAllTransactions([]);
      setPage(1);
      lastPageRef.current = 0;
      isLoadingMoreRef.current = false;
    ...
    <View style={styles.transactionItem}>
      ...
      <Text
        style={[
          ...
          {
            color:
              item.type === 'earn' ? COLORS.success : COLORS.error,
          },
        ]}
      >
        {item.type === 'earn' ? '+' : '-'}
    ...
    <>
      ...
      >
        <View style={styles.balanceHeader}>
          ...
          <Text style={styles.balanceTitle}>Điểm tích lũy</Text>
        </View>
        <Text style={styles.balanceAmount}>
          {account?.pointsBalance?.toLocaleString('vi-VN') || '0'}
        </Text>
        <Text style={styles.balanceSubtext}>điểm</Text>
      ...
      <View style={styles.infoSection}>
        <View style={styles.infoItem}>
          ...
          <Text style={styles.infoText}>
            1 điểm = 10,000₫ khi thanh toán
          </Text>
        </View>
        <View style={styles.infoItem}>
          <Ionicons name="gift-outline" size={20} color={COLORS.primary} />
          <Text style={styles.infoText}>
            Tích điểm khi mua hàng: 1% giá trị đơn hàng
          </Text>
        </View>
      </View>
